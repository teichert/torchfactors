...
class BitsModel(tx.Model[Bits]):
    def factors(self, x: Bits):
        hidden = x.environment.variable(
            'hidden',
            lambda: tx.VarField(tx.Range(10), tx.LATENT, shape=x.bits))

        length = x.bits.shape[-1]
        yield tx.LinearFactor(self.namespace('start'), hidden[..., 0])
        ...
        for i in range(length):
            yield tx.LinearFactor(self.namespace('emission'), hidden[..., i], x.bits[..., i])
