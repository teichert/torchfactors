...
def test_shape():
    ...
    v2.tensor = torch.tensor(0.0)  # "tensor(" is optional but makes mypy happy :(
...
def test_usage3():
    t = torch.ones(3, 4)
    v = Var(t, Range[4])
    v.usage = torch.tensor(ANNOTATED)
...
def test_clamp():
    ...
    v2.usage = torch.tensor(OBSERVED)
    v.clamp_annotated()
...
def test_nested():
    ...
    v = Var(t, Range[4])
    va = v[:, 3, 3:9][2, 1:3, :5]
    va.tensor = torch.tensor(1)
    assert va.shape == (2, 5)
    assert va.tensor.sum() == 10

    vb = v[2:4, :, 2:7][0, 3, 2:, :5]
    assert vb.shape == (2, 5)
