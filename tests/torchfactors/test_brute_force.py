...
def test_infer():
    ...
    factors = [
        ...
        TensorFactor(b, c)]
    bf = BruteForce()
    logz = bf.product_marginal(factors)
    assert logz.exp().isclose(torch.tensor(10. * 9 * 8), 1e-1)

    marg_a = bf.product_marginal(factors, a)
    ...
    with pytest.raises(ValueError):
        ...
        bf.product_marginals(factors, a)  # type: ignore

    # with pytest.raises(ValueError):
    #     bf.product_marginal(factors, (a, b))

    marg_a = bf.product_marginal(factors, a, normalize=False)
    assert (marg_a == math.log(9*8)).all()

    logz, marg_a = bf.product_marginals(factors, (), (a,), normalize=True)
    assert logz.exp().isclose(torch.tensor(10. * 9 * 8))
    assert (marg_a == -math.log(10)).all()

    logz, marg_a = bf.product_marginals(factors, (), (a,), normalize=False)
    assert logz.exp().isclose(torch.tensor(10. * 9 * 8))
    assert (marg_a == math.log(9*8)).all()
